...
## Literary branch convergence

On the novel branch the approved Literary Screenplay Input Bible remains the
source/world root. Specialized inputs must share one Creative Source type.
Character Art compatibility records copy arc_continuity_boundaries from the
approved Character Dramaturgy character_arc; the existing Literary validator
