...
DESIGN_NEUTRAL displays proportion/clothing readably. HERO_CASTING expresses leading-role casting importance and is legal for live action and CG. Medium, treatment and realism are separate controls; see [Visual Medium Contract](visual-medium-contract.md). The legacy HEROIC_CINEMATIC_CG adapter retains authored amplification while the shared medium compiler supplies sculpted volumes and material/rendering semantics. Keep full body/head/feet/hands/weapon readable; For HEROIC_CINEMATIC_CG, use an explicit low-angle hero composition, three-quarter orientation and motivated depth while keeping the entire design visible. A complex battle poster is not a casting proof. Character distinction and presence precede conventional attractiveness; neutral dominance need not be an angry expression.
