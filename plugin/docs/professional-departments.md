...
Every Bible has a stable ID/type/version, Work/Script/Episode and relevant Scene/Shot scope, source refs, dependency refs, status, creator capability, approval evidence, content, continuity refs and timestamps. Records distinguish `MIGRATED_FROM_R1` from `NEW_PROFESSIONAL_ELABORATION`. A migrated field cites its existing original; elaboration explains how it develops an existing intention without changing causality. `NOT_REQUIRED` requires a reason and is preferable to invented work. Contract validity, semantic design review, user approval, media observation and production authorization are separate facts.
...
| Art | character-art; costume-design; look-continuity; environment-design; environment-art; set-decoration; prop-design; animal-design | reference resolution and field ownership checks |
