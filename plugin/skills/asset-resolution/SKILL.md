...
Use `media.get_media` when an existing reference Media ID must be inspected. When no suitable Asset exists and a standard image is necessary, use `production.generate_image`; use its returned stable Media directly. Use `media.create_media` only when the generation capability returned an unregistered physical result. Then use `asset.create_asset` to register a genuinely new stable Asset. Use `asset.save_asset` only to revise an existing Asset.
