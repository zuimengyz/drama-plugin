...
Consume the reviewed Voice Bible and Voice Identity Bible against the same DPD, Director Performance Intent, exact SpokenContent and timing. Compile their recipient, spatial reach, pace/rhythm, intensity, breath support, phrase attack, articulation, emphasis, pause function, closure, coloration, release and continuity into the existing AudioPerformanceBrief; do not redesign these values. High pressure can shorten or tighten delivery without crying. Close speech need not whisper. Never replace speaker identity because the actor is tired or has cried.

For post-dubbing, `condition_audio_on_video` must consume the actual RealizedPerformanceSnapshot and its changed event timing/body load. Keep exact words and identity, reconcile execution against actual actions, then review the result again; this is not automatic retiming or lip-sync. Meaning-changing action returns to the visual owner. The legacy speech adapter rejects extended R2 briefs until a separately qualified mapping can preserve these semantics; do not strip fields or fall back to sad/angry tags. Provider controls remain outside semantic contracts.
