...
Organize persistence as **Stable Envelope + Domain Content**. Keep the parent Script ID, episode number, and title in the create envelope; use the stable Episode ID, episode number, and title when saving a revision. Put the confirmed episode job, hooks, progression, information gain, character change, and other formal episode facts in the `content` JSON object. Do not move the parent Script ID during save or hide, duplicate, or rename envelope fields inside `content`. Treat the Tool catalog as the sole machine-schema source. Submit `episode.save_episode` as a full replacement formal state, never as a patch, scratchpad, stringified JSON, or routine follow-up to create.
