...
Review every Scene before assessing generated candidates. Record SCORE_REQUIRED, SCORE_OPTIONAL, TRANSITION_ONLY, DIEGETIC_ONLY, NO_SCORE or NO_SCORE_MUST_PRESERVE (legacy SCORE_PRESENT remains readable). Silence is a complete decision. Ask what music adds that actors, dialogue and native sound do not already express. A tear does not require strings; battle does not require continuous heroic music; an available escape must not be scored as predetermined death. Do not deny an earned moment of real joy to foreshadow later loss.
