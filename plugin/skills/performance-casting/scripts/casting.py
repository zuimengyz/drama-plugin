...
p=argparse.ArgumentParser();p.add_argument('mode',choices=['schema','validate','brief','selection','compile','check-projection']);p.add_argument('--profile',type=Path);p.add_argument('--reviews',type=Path);p.add_argument('--stage',choices=['FACE','SCALE','SOCIAL','PERFORMANCE']);p.add_argument('--candidates',nargs='+');p.add_argument('--budget',type=Path);p.add_argument('--execution',default='DRY_RUN',choices=['DRY_RUN','EXECUTE','MCP']);p.add_argument('--output',type=Path);p.add_argument('--visual-plan',type=Path);p.add_argument('--variant');p.add_argument('--purpose',choices=['CANDIDATE','CALIBRATION'],default='CANDIDATE');p.add_argument('--calibration-conditions',type=Path);p.add_argument('--submitted-prompt',type=Path);p.add_argument('--route-context',type=Path);a=p.parse_args()
if a.route_context and a.mode not in {'compile','check-projection'}:p.error('route-context currently supports compile/check-projection only; route approval requires independent review')
if a.mode=='schema':result=RoleArchetypeProfile.model_json_schema(by_alias=True)
 ...
 if a.mode in {'compile','check-projection'}:
  ...
  result=compile_visual_discriminants(profile,visual,a.variant,a.stage,purpose=a.purpose,calibration_conditions=json.loads(a.calibration_conditions.read_text()) if a.calibration_conditions else None,route_context=RouteCastingContext.model_validate_json(a.route_context.read_text()) if a.route_context else None)
