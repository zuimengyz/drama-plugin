...
Express an approved Scene through minimal necessary, narratively motivated, continuous, provider-agnostic production-ready visual coverage. Plan the coverage strategy before individual Shots; give every retained Shot a clear purpose, executable subject/action/blocking, intentional camera language, and stable entry/exit state. Do not redesign the Scene or produce media.
...
Before drafting, read [Professional Shot Planning](references/planning.md) and apply it. Create an internal coverage plan that inherits rather than repairs the Scene; define the coverage strategy, necessary observations and withheld/revealed information, performance/action coverage, Shot economy, and for each retained Shot its narrative purpose, subject, action, blocking, framing/size, angle, movement, composition, dialogue/reaction role, rhythm/duration, entry/exit visual state, continuity, stable references, and generation feasibility. Keep it in Agent Run Context or temporary working state. Do not call `shot.create_shot` or `shot.save_shot` to store it.
...
Before any write, read [Shot Review and Revision](references/review.md) and apply the entire coverage rubric. Critical checks cover Scene fidelity, coverage strategy, narrative/dramatic function, subject/action/blocking, framing, camera behavior, composition, dialogue/reaction coverage, rhythm, screen direction/axis/eyeline and spatial continuity, action/performance/asset/wardrobe/prop/temporal continuity, generation feasibility, coverage economy, and downstream production readiness. Unmotivated or redundant coverage, unresolved continuity, unproducible complexity, or failure to cover the Scene turn is critical. Mark Review PASS only when every critical check passes; otherwise mark Review FAIL.
