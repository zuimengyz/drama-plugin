...
A failed image has at most one targeted correction; passed images cannot be reserved
again. Two consecutive failures or the same major category twice in the last three
reviews pause production automatically. Use `resume --input remediation.json` only
after reviewing the root cause; its `reason` records concrete corrective evidence.
It preserves attempts and budgets and requires affected representatives to pass.
A second failed attempt remains stopped. Model/reference/Shot changes need explicit
replanning and new qualification; carry exhausted Shot budgets forward, never
restart a campaign to erase failure history. Do not mechanically export a repair batch.
...
Stage limits are at most three initial images with one revision each and two total
