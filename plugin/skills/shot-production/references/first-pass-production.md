...
Inspect the selected adapter's actual reference wiring, text/seed slots and output dimensions. Reject template residue, incompatible shape or unverified derived dimensions. Provider graph encodings belong to the [conditional Host guide](../../../docs/visual-provider-host-integration.md#conditional-template-inspection), not to creative direction.
...
Use the current authorized stage limits for all created images and video
attempts, including fallbacks. Historical campaign counts do not supply a new budget. The Host-specific adapter guide provides the
