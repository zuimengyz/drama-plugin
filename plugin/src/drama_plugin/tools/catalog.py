...
def build_tool_registry(memory: MemoryProvider, asset: AssetProvider, research: ResearchProvider, production: ProductionProvider, media: MediaProvider, context: ContextProvider, voice: VoiceProvider, role_dubbing: RoleDubbingProvider) -> ToolRegistry:
    ...
    specs = [
        ...
        _domain_tool("media.list_media", "List production-discoverable media with optional filters; DEBUG artifacts require explicit inclusion.", media.list_media, list[Media], optional={"media_type": MediaType | None, "work_id": str | None, "purpose": str | None, "source_ref": str | None, "include_debug": bool}, defaults={"include_debug": False}),
        ...
        _domain_tool("media.resolve_media", "Resolve durable Drama-managed media to a temporary Drama Service content URL.", media.resolve_media, MediaResolveResult, required={"media_id": str}),
