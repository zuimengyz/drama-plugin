...
def _domain_tool(code: str, description: str, handler: ToolHandler, output: Any, *, required: Mapping[str, Any] | None = None, optional: Mapping[str, Any] | None = None) -> ToolDefinition:
    return tool(code, description, handler, input_schema=object_schema(required=required, optional=optional), output_schema=schema_for(output))
...
def build_tool_registry(memory: MemoryProvider, asset: AssetProvider, research: ResearchProvider, production: ProductionProvider, media: MediaProvider, context: ContextProvider) -> ToolRegistry:
    ...
    specs = [
        _domain_tool("work.create_work", "Create a literary work as persistent memory.", memory.create_work, Work, required={"title": str}, optional={"description": str | None, "content": dict[str, Any] | None}),
        _domain_tool("work.get_work", "Read a work by stable ID.", memory.get_work, Work, required={"work_id": str}),
        _domain_tool("work.save_work", "Save the formal state of an existing work.", memory.save_work, Work, required={"work": Work}),
        ...
        _domain_tool("script.create_script", "Create a script under a work.", memory.create_script, Script, required={"work_id": str, "title": str}, optional={"content": dict[str, Any] | None}),
        _domain_tool("script.get_script", "Read a script by stable ID.", memory.get_script, Script, required={"script_id": str}),
        _domain_tool("script.save_script", "Save the formal state of an existing script.", memory.save_script, Script, required={"script": Script}),
        _domain_tool("script.list_scripts", "List scripts under a work.", memory.list_scripts, list[Script], required={"work_id": str}),
        _domain_tool("episode.create_episode", "Create an episode under a script.", memory.create_episode, Episode, required={"script_id": str, "number": int, "title": str}, optional={"content": dict[str, Any] | None}),
        _domain_tool("episode.get_episode", "Read an episode by stable ID.", memory.get_episode, Episode, required={"episode_id": str}),
        _domain_tool("episode.save_episode", "Save the formal state of an existing episode.", memory.save_episode, Episode, required={"episode": Episode}),
        ...
        _domain_tool("scene.create_scene", "Create a scene under an episode.", memory.create_scene, Scene, required={"episode_id": str, "number": int, "heading": str}, optional={"content": dict[str, Any] | None}),
        _domain_tool("scene.get_scene", "Read a scene by stable ID.", memory.get_scene, Scene, required={"scene_id": str}),
        _domain_tool("scene.save_scene", "Save the formal state of an existing scene.", memory.save_scene, Scene, required={"scene": Scene}),
        ...
        _domain_tool("shot.create_shot", "Create a shot under a scene.", memory.create_shot, Shot, required={"scene_id": str, "number": int, "description": str}, optional={"duration_seconds": float | None, "content": dict[str, Any] | None}),
        _domain_tool("shot.get_shot", "Read a shot by stable ID.", memory.get_shot, Shot, required={"shot_id": str}),
        _domain_tool("shot.save_shot", "Save the formal state of an existing shot.", memory.save_shot, Shot, required={"shot": Shot}),
        _domain_tool("shot.list_shots", "List shots under a scene with optional structural filters.", memory.list_shots, list[Shot], required={"scene_id": str}, optional={"shot_no": int | None, "shot_type": str | None, "character": str | None}),
        ...
        _domain_tool("asset.create_asset", "Register an agent-approved stable reusable asset.", asset.create_asset, Asset, required={"asset_type": AssetType, "name": str}, optional={"description": str | None, "reference_media_ids": list[str] | None}),
        _domain_tool("asset.get_asset", "Read an asset by stable ID.", asset.get_asset, Asset, required={"asset_id": str}),
        _domain_tool("asset.save_asset", "Save the formal state of an existing asset.", asset.save_asset, Asset, required={"asset": Asset}),
        ...
        _domain_tool("media.create_media", "Register a stable physical media reference.", media.create_media, Media, required={"media_type": MediaType, "mime_type": str, "storage_key": str}, optional={"metadata": dict[str, Any] | None}),
        _domain_tool("media.get_media", "Read media metadata by stable ID.", media.get_media, Media, required={"media_id": str}),
        _domain_tool("media.save_media", "Save the formal metadata of existing media.", media.save_media, Media, required={"media": Media}),
