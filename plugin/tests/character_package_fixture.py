...
def make_package(root,version='v1',status='DRAFT',identity='Synthetic actor',routes=None,historical_role='Fictional worker'):
    ...
    if routes is not None:
        data['manifest']['supportedRoutes']=list(routes)
        data['visualExpression']['routes']={}
        for route in routes:
            live=route=='live_action_realist'
            data['visualExpression']['routes'][route]=dict(
                visualRoute='live_action_realist' if live else 'stylized_cinematic_cg',
                visualLanguage=route.upper(), proportionPolicy='REAL_HUMAN' if live else 'GROUNDED_DESIGNED',
                performancePolicy='HUMAN_PERFORMABLE' if live else 'CG_AUTHORED',
                equipmentPolicy='WEARABLE_EXECUTABLE' if live else 'FUNCTIONAL_GROUNDED',
                cameraPolicy='Scene owned',designIntent='Working identity',identityAnchors=['Attentive hands'])
