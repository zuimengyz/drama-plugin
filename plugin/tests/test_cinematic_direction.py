...
def test_registry_preserves_cinematic_skill_with_departments_and_no_new_domain_tools():
    ...
    assert {s.code for s in p.skills.list()} == expected | department_skills | {'character-art', 'costume-design', 'environment-design', 'environment-art', 'set-decoration'} | {'character-embodiment', 'character-external-driver', 'cinematic-direction', 'production-design', 'performance-casting', 'authorial-voice', 'director', 'music-direction'}
